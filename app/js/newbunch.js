...
document.getElementById('prompt-lang')
    .addEventListener('change', showPinYinMenu);
document.getElementById('answer-lang')
    .addEventListener('change', showPinYinMenu);
...
function showPinYinMenu() {
  ...
  let menuShown = false;
  if (document.getElementById('prompt-lang').value == 'zh-CN' ||
  ...
  if (document.getElementById('answer-lang').value == 'zh-CN' ||
...
function addPinYinText(e) {
  ...
  if (val.trim() != '') {
    ...
    // no
    e.target.value = val + ` (${pinyinUtil.getPinyin(val, ' ', true, false)})`;
  }
}
