...
function showAnswer() {
	if (bunchSettings.questionType.flashcard) {
		...
		document
			.getElementById("answer-image-container")
...
function genTestMC(pair, index) {
	...
	if (document.getElementById("standard-test").checked) {
		...
		questionPromptImage = pair.promptImage;
		questionAnswerImage = pair.answerImage;
		choiceType = "answer";
	} else if (document.getElementById("reversed-test").checked) {
		questionPrompt = pair.answer;
		questionAnswer = pair.prompt;
		questionPromptImage = pair.answerImage;
		questionAnswerImage = pair.promptImage;
		choiceType = "prompt";
	...
	let choice0Image, choice1Image, choice2Image;
	let answerIndex = Math.floor(Math.random() * 3);

	// Get the image property name to use for choices (either answerImage or promptImage)
	const choiceImageProp = choiceType + "Image";
	...
	switch (answerIndex) {
		case 0:
			...
			choice0Image = questionAnswerImage;
			choice1Image = pairs[randIndex1][choiceImageProp];
			choice2Image = pairs[randIndex2][choiceImageProp];
			break;
		case 1:
			choice0 = pairs[randIndex1][choiceType];
			choice1 = questionAnswer;
			choice2 = pairs[randIndex2][choiceType];
			choice0Image = pairs[randIndex1][choiceImageProp];
			choice1Image = questionAnswerImage;
			choice2Image = pairs[randIndex2][choiceImageProp];
			break;
		case 2:
			choice0 = pairs[randIndex1][choiceType];
			...
			choice0Image = pairs[randIndex1][choiceImageProp];
			choice1Image = pairs[randIndex2][choiceImageProp];
			choice2Image = questionAnswerImage;
			break;
	}

	const html = `<div class="test-MC" answer="${answerIndex}" selected=-1>
    <div class="test-MC-prompt">${questionPrompt}</div>
    <div class="test-MC-prompt-image-container study-image-container" data-image="${questionPromptImage}"></div>
    <div class="test-MC-choice-container">
        <button class="test-MC-choice" num=0>
            <div class="test-MC-choice-text">${choice0}</div>
            <div class="test-MC-choice-image-container study-image-container" data-image="${choice0Image}"></div>
            <div class="correct-indicator undisplay">&#10004;</div>
        </button>
        <button class="test-MC-choice" num=1>
            <div class="test-MC-choice-text">${choice1}</div>
            <div class="test-MC-choice-image-container study-image-container" data-image="${choice1Image}"></div>
            <div class="correct-indicator undisplay">&#10004;</div>
        </button>
        <button class="test-MC-choice" num=2>
            <div class="test-MC-choice-text">${choice2}</div>
            <div class="test-MC-choice-image-container study-image-container" data-image="${choice2Image}"></div>
            <div class="correct-indicator undisplay">&#10004;</div>
        </button>
...
function genTestTyped(pair) {
	let questionPrompt, questionAnswer, questionPromptImage;
	if (document.getElementById("standard-test").checked) {
		questionPrompt = pair.prompt;
		questionAnswer = pair.answer;
		questionPromptImage = pair.promptImage;
    ...
    <div class="test-typed-prompt">
        ${questionPrompt}
        <div class="test-typed-prompt-image-container study-image-container" data-image="${questionPromptImage}"></div>
    </div>
	...
	if (isTrue) {
        ...
        <div class="test-TF" answer="true" state="true">
            <div class="test-TF-prompt">
                ...
                <div class="test-TF-prompt-image-container study-image-container" data-image="${questionPromptImage}"></div>
            </div>
            <button class="test-TF-button">=</button>
            <div class="test-TF-answer">
                <div>${questionAnswer}</div>
                <div class="test-TF-answer-image-container study-image-container" data-image="${questionAnswerImage}"></div>
            </div>
		...
		if (randPrompt) {
			const difPromptImage = reversed
				? pairs[indexDif].answerImage
				: pairs[indexDif].promptImage;
			const html = `
                <div class="test-TF" answer="false" state="true">
                    <div class="test-TF-prompt">
                        <div>
                            ...
                            <div class="test-TF-prompt-image-container study-image-container" data-image="${difPromptImage}"></div>
                        </div>
                    </div>
                    <button class="test-TF-button">=</button>
                    <div class="test-TF-answer">
			...
			const html = `
                <div class="test-TF" answer="false" state="true">
                    ...
                    <div class="test-TF-answer">
                        ...
                        <div class="test-TF-answer-image-container study-image-container" data-image="${difAnswerImage}"></div>
                    </div>
...
function displayCardImages() {
	if (
		...
		const promptImageContainer = document.getElementById(
			"prompt-image-container",
		);
		const answerImageContainer = document.getElementById(
		...
		const promptImageFilename = currentReversed
			? currentPair.answerImage
			: currentPair.promptImage;
		const answerImageFilename = currentReversed
			? currentPair.promptImage
			: currentPair.answerImage;
...
function loadTestImages() {
	// Find all image containers in the test mode
	const imageContainers = document.querySelectorAll(
		".study-image-container[data-image]",
	);

	imageContainers.forEach((container) => {
		const filename = container.getAttribute("data-image");
		if (filename && filename !== "undefined") {
			loadStudyImage(filename, container);
		}
	});
}
//#endregion
